...
import sys 
sys.path.append("../opt") 
from modelutil.model_utils import (
    warmup, 
    decode_prompt,
    decode_prompts,
    get_wikitext2,
...
class ParserConfig:
  def __init__(self):
    ...
    self.quant_mode = "w8a8"
    self.dataset = "raw"
    self.amdort = False
    self.profile = False
    self.impl = "v0"
    self.task = "decode"
    self.num_intra_op_threads = 0
    self.num_inter_op_threads = 0
    self.intra_op_spinning = "0"
    self.inter_op_spinning = "0"

class ParserConfig:
  def __init__(self):
    self.local_path = r"C:\\Users\\V3\\Desktop\\RyzenAI-SW\\example\\transformers\\models\\opt-onnx\\opt-1.3b_ortquantized"
    self.target = "aie"
    self.perplexity = False
    self.model_name = "opt-1.3b"
    self.quant_mode = "w8a8"
